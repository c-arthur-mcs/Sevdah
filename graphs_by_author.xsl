...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="cam:cyr-to-lat">
        ...
        <xsl:variable name="one-to-one" as="xs:string"
            select="$apply-cap-dzh => translate('абвгдђежзијклмнопрстћуфхцчшАБВГДЂЕЖЗИЈКЛМНОПРСТЋУФХЦЧШ', 'abvgdđežzijklmnoprstćufhcčšABVGDĐEŽZIJKLMNOPRSTĆUFHCČŠ')"/>
            ...
            <body>
                ...
                <section>
                    <div class="info">
                        <xsl:for-each-group select="$all-docs/*"
                            ...
                            <button class="author-buttons"
                                onclick="{current-grouping-key() ! cam:cyr-to-lat(.) ! translate(., ', ', '_') ! translate(., '-', '')}()">
                                <span class="en" style="display:block">
                                    <xsl:value-of
                                        select="current-grouping-key() ! translate(., '_', ' ') ! cam:cyr-to-lat(.)"
                                    />
                                </span>
                                <span class="bs-cy" style="display:none">
                                    <xsl:value-of
                                        select="current-grouping-key() ! translate(., '_', ' ')"/>
                                </span>
                    ...
                    <xsl:for-each-group select="$all-docs/*"
                        ...
                        <div
                            class="info {current-grouping-key() ! cam:cyr-to-lat(.) ! translate(., ', ', '_') ! translate(., '-', '')}"
                            style="display:none">
                            <h2 class="en" style="display:block">
                                <xsl:value-of select="current-grouping-key() ! cam:cyr-to-lat(.)"/>
                            </h2>
                            <h2 class="bs-cy" style="display:none">
                                <xsl:value-of select="current-grouping-key()"/>
                            </h2>
                            <h2 class="bs-la" style="display:none">
                                <xsl:value-of select="current-grouping-key() ! cam:cyr-to-lat(.)"/>
                            </h2>
                            <svg xmlns="http://www.w3.org/2000/svg" height="500" width="1000">
                                ...
                                <text x="50" y="475" fill="white">
                                    ...
                                    <xsl:text> / </xsl:text>
                                    <xsl:value-of
                                        select="current-grouping-key() ! cam:cyr-to-lat(.)"/>
                                    <xsl:text>  - Total words: </xsl:text>
                                    <xsl:value-of select="$total-words"/>
                                </text>
                                <!-- LANGUAGE SPECIFIC -->
                                <!-- TURKIC -->
                                <rect fill="#FFC436" x="{50+$bar-spacing}"
                                    y="{450-(4*$pc-turkic*100)}" width="{$bar-width}"
                                    height="{4*($pc-turkic*100)}"/>
                                <text fill="white" x="{50+$bar-spacing}"
                                ...
                                <!-- LATIN -->
                                <rect fill="#FFC436"
                                    x="{50+$bar-spacing+(2*($bar-width+$bar-spacing))}"
                                ...
                                <!-- GREEK -->
                                <rect fill="#FFC436"
                                    x="{50+$bar-spacing+(3*($bar-width+$bar-spacing))}"
                                ...
                                <!-- HUNGARIAN -->
                                <rect fill="#FFC436"
                                    x="{50+$bar-spacing+(4*($bar-width+$bar-spacing))}"
                                ...
                                <!-- GERMANIC -->
                                <rect fill="#FFC436"
                                    x="{50+$bar-spacing+(5*($bar-width+$bar-spacing))}"
                                ...
                                <!-- ITALIAN -->
                                <rect fill="#FFC436"
                                    x="{50+$bar-spacing+(6*($bar-width+$bar-spacing))}"
                    ...
                    <div class="info">
                        ...
                        <table>
                            <tr>
                                <th>
                                    <span class="en" style="display:block">Name</span>
                                    <span class="bs-cy" style="display:none">Има</span>
                                    <span class="bs-la" style="display:none">Ima</span>
                                </th>
                                <th>
                                ...
                                <th>
                                    <span class="en" style="display:block">Other</span>
